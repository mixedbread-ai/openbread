...
describe("dynamicTools", () => {
  ...
  describe("list_api_endpoints", () => {
    ...
    it("should be case insensitive in search", async () => {
      ...
      result.tools.forEach(
        (tool: { name: string; resource: string; tags: string[] }) => {
          expect(
    ...
    it("should filter endpoints by description", async () => {
      ...
      expect(result.tools[0].description).toBe(
        "Test endpoint for user_endpoint"
      );
    });
  ...
  describe("get_api_endpoint_schema", () => {
    it("should return schema for existing endpoint", async () => {
      const content = await toolsMap.get_api_endpoint_schema.handler(
        fakeClient,
        {
    ...
    it("should throw error for non-existent endpoint", async () => {
      await expect(
        toolsMap.get_api_endpoint_schema.handler(fakeClient, {
    ...
    it("should throw error when no endpoint provided", async () => {
      await expect(
        toolsMap.get_api_endpoint_schema.handler(fakeClient, undefined)
      ).rejects.toThrow("No endpoint provided");
    });
  });
  ...
  describe("invoke_api_endpoint", () => {
    ...
    it("should throw error when no arguments provided", async () => {
      await expect(
        toolsMap.invoke_api_endpoint.handler(fakeClient, undefined)
      ).rejects.toThrow("No endpoint provided");
    });
